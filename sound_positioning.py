...
def position_sound_custom_2d(handle,  listener_x,  listener_y,  source_x,  source_y, theta, pan_step,  volume_step,  behind_pitch_decrease,  behind_volume_decrease,  start_pan,  start_volume,  start_pitch):
 delta_x=0
 delta_y=0
 final_pan=start_pan
 final_volume=start_volume
 ...
 # Then, we calculate the delta between the listener and the source.
 if(source_x<listener_x):
...
def position_sound_3d(handle,  listener_x,  listener_y,  listener_z,  source_x,  source_y,  source_z,  rotation, pan_step,  volume_step,  behind_pitch_decrease,  behind_volume_decrease=0):
 position_sound_custom_3d(handle, listener_x, listener_y, listener_z, source_x, source_y, source_z, rotation, pan_step, volume_step, behind_pitch_decrease, behind_volume_decrease, 0.0, 0.0, 0.0, 0.0, 100.0)

def position_sound_custom_3d(handle,  listener_x,  listener_y,  listener_z,  source_x,  source_y,  source_z,  theta, pan_step,  volume_step,  behind_pitch_decrease,  behind_volume_decrease,  pitch_step,  pitch_range,  start_pan,  start_volume,  start_pitch):
 delta_x=0
 delta_y=0
 delta_z=0
 final_pan=start_pan
